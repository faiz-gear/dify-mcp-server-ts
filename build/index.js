...
const difyParametersMap = new Map();
// cache name app sks
const appSkMap = new Map();
// Initialize Dify clients
const difyClients = new Map();
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    for (const client of difyClients.values()) {
        try {
            ...
            const inputSchema = convertDifyParametersToJsonSchema(parameters);
            // Cache Dify parameters
...
// Validate input parameters
const validateInputParameters = (args, difyParameters) => {
    const schema = z.object(Object.fromEntries(difyParameters.user_input_form.map((form) => {
 ...
 */
const convertDifyParametersToJsonSchema = (parameters) => {
    const inputSchema = {
        type: 'object',
        properties: {},
        required: []
...
const isTextInput = (input) => {
    return input['text-input'] !== undefined;
};
const isParagraphInput = (input) => {
    return input['paragraph-input'] !== undefined;
};
const isSelectInput = (input) => {
    return input['select-input'] !== undefined;
};
